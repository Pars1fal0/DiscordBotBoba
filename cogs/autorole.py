...
class AutoRole(commands.Cog):
    ...
    @app_commands.command(name="setwelcome", description="Установить канал для приветственных сообщений")
    @app_commands.describe(channel="Канал для приветственных сообщений")
    ...
    async def set_welcome_channel(self, interaction: discord.Interaction, channel: discord.TextChannel):
        ...
        embed = discord.Embed(
            ...
            description=f"Теперь приветственные сообщения будут отправляться в {channel.mention}",
            color=discord.Color.green()
        )
    ...
    async def set_welcome_channel_error(self, interaction: discord.Interaction, error):
        if isinstance(error, app_commands.MissingPermissions):
            await interaction.response.send_message(
                "❌ У тебя нет прав `Управление сервером`.",
                ephemeral=True
            )
        else:
